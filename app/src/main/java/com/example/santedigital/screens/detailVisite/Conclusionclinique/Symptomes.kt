...
import com.example.santedigital.ui.theme.ViewModel.Syptom
import com.example.santedigital.ui.theme.ViewModel.syptomSharedViewModel
import com.example.santedigital.ui.theme.gilroyFont


@Composable
fun Symptomes(
    syptomSharedViewModel: syptomSharedViewModel = viewModel()
) {
    val allSyptom = syptomSharedViewModel.getAllSyptom()
    var idSymptomes by remember { mutableStateOf(0) }
    ...
    Column() {
        ...
        Row(modifier  = Modifier.fillMaxWidth(),horizontalArrangement = Arrangement.Center) {
            FloatingActionButton(backgroundColor = Purple500, contentColor = Color.White,onClick = {
                ...
                syptomSharedViewModel.addSyptom(syptom = Syptom(id = idSymptomes , label = "string" ))
            }) {
